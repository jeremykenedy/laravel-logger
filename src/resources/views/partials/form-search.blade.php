    ...
    <div class="row mb-3">
        @if(in_array('description',explode(',', config('LaravelLogger.searchFields'))))
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <input type="text" name="description" value="{{request()->get('description') ? request()->get('description'):null}}" class="form-control" placeholder="{{ trans('LaravelLogger::laravel-logger.placeholders.description') }}">
            </div>
        @endif
        ...
        @if(in_array('route',explode(',', config('LaravelLogger.searchFields'))))
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <input type="text" name="route" class="form-control" value="{{request()->get('route') ? request()->get('route'):null}}" placeholder="{{ trans('LaravelLogger::laravel-logger.placeholders.route') }}">
            </div>
        @endif
        @if(in_array('ip',explode(',', config('LaravelLogger.searchFields'))))
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <input type="text" name="ip_address" class="form-control" value="{{request()->get('ip_address') ? request()->get('ip_address'):null}}" placeholder="{{ trans('LaravelLogger::laravel-logger.placeholders.ip_address') }}">
            </div>
        @endif
